...
function fetchServiceList($id,$type)
{
	$responseDoc = new DOMDocument();
        ...
        if($type=='Product')
        {
                $fetchSql = "SELECT vtiger_products.*, vtiger_crmentity.*, vtiger_productcf.* FROM vtiger_products JOIN vtiger_crmentity ON vtiger_products.productid=vtiger_crmentity.crmid JOIN vtiger_productcf ON vtiger_products.productid=vtiger_productcf.productid WHERE vtiger_productcf.productid = ".$id;
	...
	if($type=="Service")
	{
		$fetchSql = "SELECT vtiger_service.*, vtiger_crmentity.*, FROM vtiger_service JOIN vtiger_crmentity ON vtiger_service.serviceid=vtiger_crmentity.crmid WHERE vtiger_service.productid = ".$id;
                $resultSql = mysql_query($fetchSql);
